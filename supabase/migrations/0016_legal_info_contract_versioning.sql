-- ══════════════════════════════════════════════════════════════════════════
-- Génération Capable — Informations légales dynamiques & versioning contrats
-- ══════════════════════════════════════════════════════════════════════════
-- Complète 0015 :
--   1. company_legal_info — identité légale de la Société, modifiable depuis
--      Administration → Informations légales (plus AUCUNE valeur "à
--      compléter" codée en dur dans les contrats générés).
--   2. contract_templates — le TEXTE des contrats devient une donnée versionnée
--      (v1, v2, v3...) plutôt qu'une constante dans le code : publier une
--      nouvelle version validée par un juriste ne demande plus de déploiement.
--
-- Écriture réservée admin dans les deux cas, comme le reste des tables de
-- configuration sensibles de ce schéma (voir "admins", 0003) : RLS activée,
-- volontairement AUCUNE policy insert/update pour anon/authenticated — les
-- Netlify Functions admin (admin-legal-info.js, admin-contract-templates.js)
-- vérifient elles-mêmes l'appartenance à "admins" avant d'écrire en
-- service_role, exactement comme admin-verify.js/ambassador-data.js.

-- ──────────────────────────────────────────────────────────────────────────
-- 1. COMPANY_LEGAL_INFO — ligne UNIQUE (id fixé à 1), toute l'identité
--    légale utilisée pour remplir les contrats.
-- ──────────────────────────────────────────────────────────────────────────
create table if not exists public.company_legal_info (
  ...
  email             text,
  phone             text,
  logo_path         text,       -- chemin Storage (bucket "compliance-documents", dossier "legal/")
  signature_path    text,       -- image de la signature officielle de la Société
  updated_at        timestamptz not null default now(),
  updated_by        uuid references auth.users(id)
);

alter table public.company_legal_info enable row level security;
-- Aucune policy anon/authenticated : lecture (pour remplir un contrat) et
-- écriture passent exclusivement par le service_role des Netlify Functions,
-- qui vérifient elles-mêmes l'identité de l'appelant au préalable.

insert into public.company_legal_info (id) values (1) on conflict (id) do nothing;


-- ──────────────────────────────────────────────────────────────────────────
-- 2. CONTRACT_TEMPLATES — texte intégral versionné, par rôle. Jamais réécrit
--    (une "nouvelle version" = une nouvelle ligne) : la ligne historique
--    référencée par un contract_signatures.contract_version existant ne doit
--    jamais changer de contenu après coup.
-- ──────────────────────────────────────────────────────────────────────────
create table if not exists public.contract_templates (
  id             uuid primary key default gen_random_uuid(),
  role           text not null check (role in ('vendeur','ambassadeur')),
  version        text not null,          -- ex: 'v1', 'v2'...
  body           text not null,          -- texte intégral, jetons {{PLACEHOLDER}}
...
create index if not exists contract_templates_current_idx on public.contract_templates (role) where is_current = true;

alter table public.contract_templates enable row level security;
drop policy if exists contract_templates_select_admin on public.contract_templates;
create policy contract_templates_select_admin on public.contract_templates for select
  using (is_current_user_admin());
-- Écriture réservée service_role (admin-contract-templates.js) : publier une
-- version doit atomiquement désactiver l'ancienne is_current, ce qu'une
-- simple policy insert ne peut pas garantir.


-- ──────────────────────────────────────────────────────────────────────────
-- 3. SEED — version v1 des deux contrats, texte identique à celui déjà en
--    production dans _lib/compliance/contracts.js au moment de cette
--    migration (recopié programmatiquement, jamais retapé à la main, pour
--    garantir un octet-pour-octet identique — voir contracts.js pour le
--    mécanisme de repli si cette table était vide).
-- ──────────────────────────────────────────────────────────────────────────
insert into public.contract_templates (role, version, body, is_current, change_notes)
values ('vendeur', 'v1', 'Génération Capable
CONTRAT DE VENDEUR
...
---
Projet de contrat établi à titre indicatif par assistance IA, sur la base des éléments fournis par Génération Capable. Ce document ne constitue pas un avis juridique et doit impérativement être relu, complété et validé par un avocat ou juriste avant toute signature.', true, 'Version initiale — reprise telle quelle du document fourni par Génération Capable, articles 16-17 ajoutés (sécurité/fraude, valeur juridique de la signature électronique).')
on conflict (role, version) do nothing;

insert into public.contract_templates (role, version, body, is_current, change_notes)
values ('ambassadeur', 'v1', 'Génération Capable
...
--   and tablename in ('company_legal_info','contract_templates');
--   select id from public.company_legal_info; -- doit renvoyer 1 ligne (id=1)
-- ──────────────────────────────────────────────────────────────────────────
