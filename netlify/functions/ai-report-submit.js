    ...
    for (const admin of Array.isArray(admins) ? admins : []) {
      ...
      for (const sub of Array.isArray(subs) ? subs : []) {
        const r = await sendToSubscription(sub, {
          title: `🤖 Nouveau résultat IA — ${report.tool}`,
          body: `${report.user_email} vient d'utiliser ${report.tool}. Ouvre la boîte IA pour faire ton retour.`,
          tag: 'ai-report-' + report.id,
          url: '/admin-ai-reports.html',
        });
        if (r.ok) sent = true;
        if (r.expired) {
          await supabaseAdminRequest(`/rest/v1/push_subscriptions?id=eq.${sub.id}`, {
      ...
      await supabaseAdminRequest('/rest/v1/notification_log', {
        ...
        body: JSON.stringify({
          audience: 'admin', user_id: user.id, category: 'admin.ai.report',
          event_key: `ai-report:${report.id}`, title: `Nouveau résultat IA — ${report.tool}`,
          body: `${report.user_email} a généré un résultat IA.`,
          status: sent ? 'sent' : 'skipped_no_subscription',
          metadata: { report_id: report.id, tool: report.tool }
        })
      });
    }
  } catch (e) {
    console.error('[ai-report-submit] notification admin:', e);
  }
}
...
exports.handler = async (event) => {
  ...
  const r = await supabaseAdminRequest('/rest/v1/ai_coaching_reports', {
    method: 'POST',
  ...
  if (!r.ok) {
    ...
    console.error('[ai-report-submit] Supabase:', r.status, detail);
    return jsonResponse(500, { error: 'SAVE_FAILED' });
  }
  const rows = await r.json();
  const saved = Array.isArray(rows) ? rows[0] : null;
  if (saved) await notifyAdmins(saved);
